...
def extract_movie_details(movie_link):
    ...
    if movie_soup is not None:
        ...
        find_title = movie_soup.find("div", attrs={"class": "title_wrapper"})
        if find_title is not None:
            title = find_title.find("h1").get_text()

        title_wrapper = movie_soup.find("div", attrs={"class": "title_wrapper"})
        if title_wrapper is not None:
            date = title_wrapper.find("a", attrs={"title": "See more release dates"}).get_text().strip()
